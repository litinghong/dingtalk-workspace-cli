...
- `/workspace/skills/dws/bin/dws`

执行 `dws` 前必须先按以下顺序解析命令路径：

1. 若 `/workspace/skills/dws/bin/dws` 存在且可执行，优先使用该绝对路径
2. 否则若环境变量 `DWS_BIN` 已设置且可执行，使用 `"$DWS_BIN"`
3. 否则回退到 `PATH` 里的 `dws`

示例（在当前仓库根目录执行）：

```bash
if [ -x "skills/bin/dws" ]; then
  DWS_CMD="$(pwd)/skills/bin/dws"
elif [ -n "${DWS_BIN:-}" ] && [ -x "${DWS_BIN}" ]; then
  DWS_CMD="${DWS_BIN}"
else
  DWS_CMD="dws"
fi
